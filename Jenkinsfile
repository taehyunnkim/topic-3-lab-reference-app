    ...
    stages {
        ...
        stage('Deploy') {
            steps {
                ...
                sh "docker run -d -p '${env.EXPOSE_PORT}:3000' -e DB_USERNAME=${DB_USERNAME} -e DB_PASSWORD=${DB_PASSWORD} --network open-devsecops_default --name ${env.APP_NAME} ${AWS_ACCOUNT_ID}.dkr.ecr.${REGION}.amazonaws.com/${env.APP_NAME}"
            }
        }
    }
}